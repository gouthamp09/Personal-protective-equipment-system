...
//     setLoading(true);
//     setUploadProgress(0);
//     setLogs((prevLogs) => [...prevLogs, "⏳ Uploading image..."]);
...
//       setLogs((prevLogs) => [
//         ...prevLogs,
//         `❌ Error uploading image: ${error.response?.data?.message || error.message}`,
//       ]);
//     } finally {
//       setLoading(false);
//     }
...
//       position: "relative"
//     }}>
//       <div className="overlay" style={{ padding: "20px", borderRadius: "10px" }}>
...
//         <button onClick={handleImageUpload} className="upload-btn" disabled={loading}>
...
//     axios.post('http://localhost:5000/set-receiver-email', { email: userEmail })
//       .then(response => {
//         console.log('✅ Email set successfully:', response.data);
//       })
//       .catch(error => {
...
//   // Check if user email exists in localStorage on page load
//   useEffect(() => {
...
//       // Send email to the backend (optional)
//       axios
//         .post("http://localhost:5000/set-receiver-email", { email })
//         .then((response) => {
//           console.log("✅ Email set successfully:", response.data);
//           setLogs((prevLogs) => [...prevLogs, `📩 Receiver email set to ${email}`]);
//         })
//         .catch((error) => {
//           console.error("❌ Error setting email:", error);
//           setLogs((prevLogs) => [...prevLogs, `❌ Error setting email: ${error.message}`]);
//         });
//     } catch (error) {
//       console.error("❌ Error decoding JWT:", error);
//       setLogs((prevLogs) => [...prevLogs, `❌ Error with login: ${error.message}`]);
//     }
//   };

//   const handleLoginError = () => {
//     console.error("❌ Login Failed");
//     setLogs((prevLogs) => [...prevLogs, "❌ Google login failed"]);
//   };
